...
if __name__ == "__main__":
    ...
    for d in data:
        bst.insert(d, d * 2)
        print("Inserted ({}, {}) tuple".format(d, d*2))

    data.reverse()
    for s in data:
        print("Search for key {} returned {}".format(s, bst.search(s)))

    bst.print()
    print("After {} insertions, bst has {} elements".format(len(data), bst.search(s)))

    print("Pred of node with key 140 {}".format(bst.pred(140)))
    print("Pred of node with key 125 {}".format(bst.pred(125)))
    print("Tree max from root is {}".format(bst.max(bst.tree.root)))

    bst.delete(40)
    print("Deleted leaf 40")
    bst.print()

    bst.delete(110)
    print("Deleted internal node 110 with only 1 child")
    bst.print()

    bst.delete(140)
    print("Deleted internal node 140 with 2 children")
    bst.print()
